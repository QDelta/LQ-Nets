...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, w_nbits=None, a_nbits=None):
        ...
        self.conv1 = LiQConv(in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False, nbits=w_nbits)
        self.bn1 = nn.BatchNorm2d(planes)
        self.activ1 = nn.Sequential(nn.ReLU(True), LiQActiv(nbits=a_nbits))
        self.conv2 = LiQConv(planes, planes, kernel_size=3, stride=1, padding=1, bias=False, nbits=w_nbits)
        self.bn2 = nn.BatchNorm2d(planes)
        self.activ2 = nn.Sequential(nn.ReLU(True), LiQActiv(nbits=a_nbits))
